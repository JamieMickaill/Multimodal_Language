...
class HKT(nn.Module):
    ...
    def forward(self, input_ids, visual, acoustic, hcf, attention_mask=None, token_type_ids=None):
        ...
        #take CLS from each modality after processing with shared encoder
        text_cls = all_features_embedding[:, 0, :]
        v_cls = all_features_embedding[:, text_len, :]
        a_cls = all_features_embedding[:, text_len + v_len, :]
        h_cls = all_features_embedding[:, text_len + v_len + a_len, :]
        
        fused_output = torch.cat([text_cls, v_cls, a_cls, h_cls], dim=1)
