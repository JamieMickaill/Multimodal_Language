...
#Single layer x-attn between all concatenated features
...
class HKT(nn.Module):
    ...
    def get_params(self):
        ...
        other_params=list(self.text_model.named_parameters())+list(self.self_cross_attention.named_parameters())+list(self.fusion_fc.named_parameters())
    ...
    def forward(self, input_ids, visual, acoustic,hcf, attention_mask=None, token_type_ids=None):
        ...
        text_hcf = torch.cat((text_output,hcf_output),dim=2)
        all_encodings=torch.cat((text_output,hcf_output,visual_output,acoustic_output),dim=2)
        ...
        self_x_comb = self.self_cross_attention(all_encodings,all_encodings,attention_mask=extended_attention_mask)
        ...
        visual_embedding = F.max_pool1d(visual_output.permute(0,2,1).contiguous(), visual_output.shape[1]).squeeze(-1)
        acoustic_embedding = F.max_pool1d(acoustic_output.permute(0,2,1).contiguous(),acoustic_output.shape[1]).squeeze(-1)
        all_self_x_embedding = F.max_pool1d(self_x_comb.permute(0,2,1).contiguous(), self_x_comb.shape[1]).squeeze(-1) 

        fusion = (text_embedding, visual_embedding, acoustic_embedding,all_self_x_embedding)
        fused_hidden = torch.cat(fusion, dim=1)
