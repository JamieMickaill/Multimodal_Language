...
#Early Multi layer x-attn between AV VT AT
...
class HKT(nn.Module):
    def __init__(self, text_model, visual_model, acoustic_model, hcf_model, args, dropout=0.1, fusion_dim=128):
        ...
        self.x1_pred = nn.Linear(LANGUAGE_DIM+HCF_DIM+ACOUSTIC_DIM, 1)
        self.x2_pred = nn.Linear(LANGUAGE_DIM+HCF_DIM+VISUAL_DIM, 1)
        self.x3_pred = nn.Linear(ACOUSTIC_DIM+VISUAL_DIM, 1)
        self.text_pred = nn.Linear(LANGUAGE_DIM, 1)
    ...
    def get_params(self):
        ...
        other_params=list(self.text_audio_cross_attention.named_parameters())+list(self.text_visual_cross_attention.named_parameters())+list(self.audio_visual_cross_attention.named_parameters())+list(self.fusion_fc.named_parameters())+list(self.x1_pred.named_parameters())+list(self.x2_pred.named_parameters())+list(self.x3_pred.named_parameters())+list(self.text_pred.named_parameters())
    ...
    def forward(self, input_ids, visual, acoustic,hcf, attention_mask=None, token_type_ids=None):
        ...
        text_hcf=torch.cat((text_output,hcf),dim=2)
        ...
        #x-attn early - pre unimodal encoder
        text_audio_comb = self.text_audio_cross_attention(text_hcf, acoustic, attention_mask=extended_attention_mask)
        text_visual_comb = self.text_visual_cross_attention(text_hcf, visual, attention_mask=extended_attention_mask)
        audio_visual_comb = self.audio_visual_cross_attention(acoustic, visual, attention_mask=extended_attention_mask)

        #get predictions for early fusion x-attn
        ta_pred = self.x1_pred(text_audio_comb[:,0,:])
        tv_pred = self.x2_pred(text_visual_comb[:,0,:])
        av_pred = self.x3_pred(audio_visual_comb[:,0,:])
        text_pred = self.text_pred(text_output[:,0,:])
        ...
        fusion = (text_pred, v_cls, a_cls,h_cls,ta_pred,tv_pred,av_pred)
        fused_preds = torch.cat(fusion, dim=1)
        
        out = self.fusion_fc(fused_preds)
        
        return (out, fused_preds)
