...
class HKT_no_A(nn.Module):
    def __init__(self, text_model, visual_model, acoustic_model, hcf_model, args, dropout=0.1, fusion_dim=128):
        ...
        # self.seq_len = args.max_seq_length
        self.d_model = (LANGUAGE_DIM+VISUAL_DIM+HCF_DIM)
        self.pos_encoder = nn.Embedding(args.max_seq_length, self.d_model)
        self.norm = nn.LayerNorm(self.d_model)
    ...
    def forward(self, input_ids, visual, acoustic, hcf, attention_mask=None, token_type_ids=None):
        ...
        #concat vertically
        all_features_comb = torch.cat((t_tokens, visual, hcf), dim=2)
...
class HKT_no_V(nn.Module):
    def __init__(self, text_model, visual_model, acoustic_model, hcf_model, args, dropout=0.1, fusion_dim=128):
        ...
        # self.seq_len = args.max_seq_length
        self.d_model = (LANGUAGE_DIM+ACOUSTIC_DIM+HCF_DIM)
        self.pos_encoder = nn.Embedding(args.max_seq_length, self.d_model)
        self.norm = nn.LayerNorm(self.d_model)
    ...
    def forward(self, input_ids, visual, acoustic, hcf, attention_mask=None, token_type_ids=None):
        ...
        #concat vertically
        all_features_comb = torch.cat((t_tokens, acoustic, hcf), dim=2)
...
class HKT_no_T(nn.Module):
    def __init__(self, text_model, visual_model, acoustic_model, hcf_model, args, dropout=0.1, fusion_dim=128):
        ...
        # self.seq_len = args.max_seq_length
        self.d_model = (VISUAL_DIM+ACOUSTIC_DIM+HCF_DIM)
        self.pos_encoder = nn.Embedding(args.max_seq_length, self.d_model)
        self.norm = nn.LayerNorm(self.d_model)
    ...
    def forward(self, input_ids, visual, acoustic, hcf, attention_mask=None, token_type_ids=None):
        ...
        #concat vertically
        all_features_comb = torch.cat(( visual, acoustic, hcf), dim=2)
