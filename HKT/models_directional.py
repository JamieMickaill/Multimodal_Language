...
class DirectionalCrossAttentionLayer(nn.Module):
    def __init__(self, hidden_size, context_size, nhead=1, dropout=0.1):
        super(DirectionalCrossAttentionLayer, self).__init__()
        self.src_cross_attention = Attention(hidden_size, nhead, dropout, ctx_dim=context_size)
        self.fc = nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.ReLU())  # Adjusted here
        self.norm1 = nn.LayerNorm(hidden_size)
        self.norm2 = nn.LayerNorm(context_size)
        self.norm3 = nn.LayerNorm(hidden_size)  # Added new LayerNorm for post-attention src

        self.dropout1 = nn.Dropout(dropout)
        self.dropout2 = nn.Dropout(dropout)

    def forward(self, src, context, attention_mask=None):
        src_n = self.norm1(src)
        ctx_n = self.norm2(context)
        new_src = self.src_cross_attention(src_n, ctx_n, attention_mask=attention_mask)

        # Skip connection + dropout
        src_1 = src_n + self.dropout1(new_src)
        
        # Layer norm (adjusted here)
        src_1_n = self.norm3(src_1)
        
        # FC (adjusted here)
        src_1_fc = self.fc(src_1_n)
        
        # Skip connection + dropout again
        src_out = src_1_n + self.dropout2(src_1_fc)
...
class HKT(nn.Module):
    ...
    def forward(self, input_ids, visual, acoustic,hcf, attention_mask=None, token_type_ids=None):
        ...
        (visual_embedding, _, visual_output) = self.visual_model(visual)
        (acoustic_embedding, _, acoustic_output) = self.acoustic_model(acoustic)
        (_, _, hcf_output) = self.hcf_model(hcf)
        ...
        # visual_embedding = F.max_pool1d(visual_output.permute(0,2,1).contiguous(), visual_output.shape[1]).squeeze(-1)
        # acoustic_embedding = F.max_pool1d(acoustic_output.permute(0,2,1).contiguous(),acoustic_output.shape[1]).squeeze(-1)
        text_audio_embedding = text_audio_comb[:,0,:]
        text_visual_embedding = text_visual_comb[:,0,:]

        fusion = (text_embedding, text_audio_embedding,text_visual_embedding)
        fused_hidden = torch.cat(fusion, dim=1)
